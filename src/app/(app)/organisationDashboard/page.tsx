...
function PremiumOrganizationDashboard() {
  ...
  const saveDescription = async () => {
    ...
    try {
      const response = await axios.post('/api/updateOrganisationDescription', {
  ...
  const saveImage = async () => {
    ...
    try {
      const response = await axios.post('/api/updateOrganisationLogo', formData, {
        headers: {
      ...
      if (response.data.success) {
        setOrg({...org, logoUrl: response.data.logoUrl});
        toast({
          title: 'Success',
        ...
        toast({
          title: 'Error',
          description: response.data.message || 'Failed to update logo',
          variant: 'destructive',
        });
      }
    } catch (error) {
      ...
      toast({
        title: 'Error',
        description: 'Something went wrong while updating logo',
        variant: 'destructive',
      });
    } finally {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-900">
        <div className="flex flex-col items-center">
          ...
          <span className="text-orange-400 font-medium">Loading your dashboard...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white">
      {/* Navigation */}
      <nav className="bg-slate-800/80 backdrop-blur-md border-b border-slate-700/50 px-6 py-4 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="flex items-center gap-2 px-6 py-2 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white font-semibold rounded-lg transition-all shadow-lg hover:shadow-orange-500/25"
            >
            ...
            <button 
              ...
              className="flex items-center gap-2 px-4 py-2 text-slate-300 hover:text-white hover:bg-slate-700/50 rounded-lg transition-all"
            >
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-slate-400 text-xl">Your premium feedback management hub</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
          {/* Share Profile Card */}
          <div className="lg:col-span-2 bg-slate-800/40 backdrop-blur-sm rounded-2xl p-8 border border-slate-700/50">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-12 h-12 rounded-xl bg-orange-500/20 flex items-center justify-center">
                <Copy className="w-6 h-6 text-orange-400" />
              </div>
              <h2 className="text-2xl font-semibold text-white">Share Your Profile</h2>
            </div>
            <p className="text-slate-400 mb-6">Share this link to receive anonymous feedback from anyone</p>
            <div className="flex flex-col sm:flex-row gap-4">
              <input
                type="text"
                value={profileUrl}
                readOnly
                className="flex-1 px-4 py-3 bg-slate-700/50 border border-slate-600/50 rounded-xl text-slate-200 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all"
              />
              <button
                onClick={copyToClipboard}
                className="flex items-center justify-center gap-2 px-6 py-3 bg-orange-500 hover:bg-orange-600 text-white font-medium rounded-xl transition-all"
              >
          ...
          <div className="bg-slate-800/40 backdrop-blur-sm rounded-2xl p-8 border border-slate-700/50">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-slate-700/30 rounded-xl">
                <div>
                  <h3 className="text-white font-medium">Accept Messages</h3>
        ...
        <div className="bg-slate-800/40 backdrop-blur-sm rounded-2xl p-8 border border-slate-700/50 mb-8">
          <h2 className="text-2xl font-semibold text-white mb-8">Organization Profile</h2>
          
          <div className="flex flex-col lg:flex-row gap-8">
            {/* Organization Image */}
            <div className="flex-shrink-0">
              <div className="relative">
                <div className="w-32 h-32 rounded-2xl overflow-hidden border-2 border-slate-600">
                ...
                {isEditingImage ? (
                  <div className="absolute -bottom-2 -right-2 flex gap-2">
                    <button
                      ...
                      className="w-10 h-10 bg-green-500 hover:bg-green-600 rounded-xl flex items-center justify-center transition-all disabled:opacity-50"
                    >
                      {isSubmittingImage ? (
                  ...
                  <button
                    ...
                    className="w-10 h-10 bg-orange-500 hover:bg-orange-600 rounded-xl flex items-center justify-center transition-all absolute -bottom-2 -right-2"
                  >
            ...
            <div className="flex-1">
              <h3 className="text-3xl font-bold text-white mb-4">{org.name}</h3>
              ...
              {isEditingDesc ? (
                <div className="space-y-4">
                  <textarea
                    ...
                    className="w-full h-32 px-4 py-3 bg-slate-700/50 border border-slate-600/50 rounded-xl text-slate-200 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 resize-none transition-all"
                  ...
                  <div className="flex gap-3">
                    <button
                      ...
                      className="flex items-center gap-2 px-6 py-2 bg-green-500 hover:bg-green-600 text-white rounded-lg transition-all disabled:opacity-50"
                    >
                      {isSubmittingDesc ? (
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 px-6 py-2 bg-slate-600 hover:bg-slate-700 text-white rounded-lg transition-all"
                    >
                      <X className="w-4 h-4" />
                      Cancel
                    </button>
                ...
                <div className="relative">
                  <p className="text-slate-300 text-lg leading-relaxed pr-8">
        ...
        <div className="bg-slate-800/40 backdrop-blur-sm rounded-2xl p-8 border border-slate-700/50">
          <div className="flex justify-between items-center mb-8">
            <h2 className="text-2xl font-semibold text-white">Organization Messages</h2>
            <div className="flex gap-3">
              <button
                onClick={() => fetchMessages(true)}
                className="flex items-center gap-2 px-4 py-2 bg-slate-700/50 hover:bg-slate-700 text-slate-300 hover:text-white rounded-lg transition-all"
              >
                {isLoading ? (
                  <Loader2 className="w-4 h-4 animate-spin" />
                ) : (
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-lg transition-all disabled:opacity-50"
              >
